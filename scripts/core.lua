...
function core.get_entity_info(entity)
  ...
  -- Track entities that can change quality. Primaries are always tracked, even
  -- at max quality, so they keep feeding their surface's progression meter.
  local can_upgrade = quality_selector.has_upgrade_path(entity.quality.name)
  local should_track = can_upgrade or is_primary
...
function core.process_primary_entity(entity_info, entity)
  ...
  -- The surface meter tracks the total credits the average primary on the
  -- surface has earned, so each primary deposits its own share of that average
...
function core.batch_process_entities()
  ...
  while entities_processed < batch_size do
    ...
    -- Primaries always stay tracked so their deposits keep the surface meter moving
