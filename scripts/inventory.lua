...
Scans quality levels in round-robin fashion, building knowledge over time while
...
local function get_min_quality_level(entity_name)
  ...
  for unit_number, entity_info in pairs(storage.quality_control_entities or {}) do
  ...
  return found_any and min_level or 0
end

-- Update scan data for a specific entity type in a network
local function update_scan_data(network, entity_name)
  local network_id = network.network_id

  -- Initialize network scan data if needed
  if not storage.network_quality_scans[network_id] then
    storage.network_quality_scans[network_id] = {}
  end

  if not storage.network_quality_scans[network_id][entity_name] then
    storage.network_quality_scans[network_id][entity_name] = {
      min_quality_level = 0,
  ...
  local scan_data = storage.network_quality_scans[network_id][entity_name]

  -- Update minimum quality level based on current tracked entities
  scan_data.min_quality_level = get_min_quality_level(entity_name)
...
function inventory.check_upgrade_available(network, entity_name, current_quality)
  ...
  local scan_data = update_scan_data(network, entity_name)
  ...
  for level = quality_limit.level, current_quality.level + 1, -1 do
    ...
    for unit_number, upgrade_info in pairs(storage.pending_upgrades) do
...
-- Clean up completed or stale pending upgrades
function inventory.cleanup_pending_upgrades()
...
function inventory.check_and_complete_upgrade(entity)
  ...
  if old_unit_number and matching_upgrade then
    inventory.complete_pending_upgrade(old_unit_number)
    return true
  end
...
-- Remove pending upgrade when upgrade completes
function inventory.complete_pending_upgrade(old_unit_number)
...
function inventory.on_cancelled_upgrade(event)
  ...
  if entity.valid then
    inventory.complete_pending_upgrade(entity.unit_number)
  end
end
...
function inventory.cleanup_invalid_networks()
  ...
  -- Collect all valid network IDs
  for _, network in pairs(game.forces.player.logistic_networks) do
    valid_network_ids[network.network_id] = true
  end

  -- Remove scan data for invalid networks
  for network_id in pairs(storage.network_quality_scans) do
