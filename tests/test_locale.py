...
@pytest.mark.parametrize("locale_file", get_all_locale_files(), ids=lambda f: f"{f.parent.name}/{f.name}")
def test_all_locale_files_ai_disclaimer(locale_file):
    """Test that non-English locale files start with an AI translation disclaimer comment."""
    if not locale_file or not locale_file.exists():
        pytest.skip(f"Locale file not found: {locale_file}")
    ...
    disclaimer_message = (
        f"{locale_file.parent.name}/locale.cfg: Missing AI translation disclaimer. "
        "Non-English locale files must start with a comment (# ...) that says roughly "
        "the following, translated into the file's language: "
    ...
    assert first_line.startswith('#'), disclaimer_message
    assert 'mods.factorio.com/mod/quality-control/discussion' in first_line, disclaimer_message
    assert 'github.com/aarons/factorio-quality-control/issues' in first_line, disclaimer_message
