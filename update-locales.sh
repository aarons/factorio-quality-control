...
validate_locale() {
    ...
    # Run pytest on the specific locale file
    cd "$(dirname "$0")"
    if [ -d "tests/venv" ]; then
        source tests/venv/bin/activate
        validation_output=$(python -m pytest tests/test_locale.py -v 2>&1 || true)
        deactivate
    else
        validation_output=$(python -m pytest tests/test_locale.py -v 2>&1 || true)
    fi
    ...
    if echo "$validation_output" | grep -q "FAILED\|ERROR"; then
