...
cleanup() {
    echo ""
    echo -e "${YELLOW}Interrupted — killing background jobs...${NC}"
    kill 0 2>/dev/null
    exit 1
}
...
validate_locale() {
    ...
    if echo "$validation_output" | grep -q "0 selected"; then
        ...
        echo -e "${YELLOW}Validation failed for $lang_name. Attempting to fix (attempt $attempt/$MAX_VALIDATION_ATTEMPTS)...${NC}"
    ...
    case "$1" in
        ...
        --context)
            if [ -n "$2" ] && [ "${2:0:1}" != "-" ]; then
                ...
                echo -e "${RED}Error: --context requires a message${NC}"
                exit 1
            fi
            ;;
        --context=*)
...
Additional context from the user: $CONTEXT_MSG"
    ...
    local attempt=1
    local max_attempts=$MAX_VALIDATION_ATTEMPTS
...
if [ $failed_count -gt 0 ]; then
    echo -e "${YELLOW}⚠ Needs follow-up ($failed_count):${NC}"
    for result_file in "$TEMP_DIR"/result_*; do
        [ -f "$result_file" ] || continue
        result=$(cat "$result_file")
        status="${result%%:*}"
        if [ "$status" = "failed" ]; then
            ...
            lang_code="${rest%%:*}"
            lang_name="${rest#*:}"
            echo -e "  - ${YELLOW}$lang_code ($lang_name): validation failed after $MAX_VALIDATION_ATTEMPTS attempts${NC}"
        fi
    done
fi
